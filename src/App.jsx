import { Suspense, useRef } from 'react'
import { Canvas, useFrame, useLoader } from '@react-three/fiber'
import { ContactShadows, useGLTF, OrbitControls } from '@react-three/drei'
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader'
import { NikeAirJordan } from './NikeAirJordan.jsx'

/*
Auto-generated by: https://github.com/pmndrs/gltfjsx
Sketchfab: https://sketchfab.com/3d-models/brazillian-boot-vento-tita-6e95243a64a545aab3af43173f6b3e04
Author: https://sketchfab.com/Akaize
*/

useGLTF.preload('/scene-transformed.glb')

// function Shoe({ ...props }) {
//   const ref = useRef()
//   const { nodes, materials } = useGLTF('/nike-air-jordan.glb')
//   useFrame((state) => {
//     const t = state.clock.getElapsedTime()
//     ref.current.rotation.set(0.1 + Math.cos(t / 4.5) / 10, Math.sin(t / 4) / 4, 0.3 - (1 + Math.sin(t / 4)) / 8)
//     ref.current.position.y = (1 + Math.sin(t / 2)) / 10
//   })
//   return (
//     <group {...props} dispose={null}>
//       <group ref={ref}>
//         <group position={[-0.16, 0, -0.22]} rotation={[0, -Math.PI / 2, 0]}>
//           <mesh castShadow geometry={nodes.Object_115.geometry} material={materials['Material.002']} />
//           <mesh castShadow receiveShadow geometry={nodes.Object_119.geometry} material={materials['Material.001']} />
//         </group>
//       </group>
//     </group>
//   )
// }

export default function App() {
  return (
    <Canvas shadows dpr={[1, 2]} camera={{ position: [0, 0, 1.1], fov: 50 }}>
      <OrbitControls enablePan={false} minPolarAngle={0} maxPolarAngle={Math.PI / 2.25} target={[0.25, 0, 0]} />
      <ambientLight intensity={4} />
      <spotLight position={[1, 6, 1.5]} angle={0.2} penumbra={1} intensity={3} castShadow shadow-mapSize={[2048, 2048]} color="#fbf4e1" />
      <spotLight position={[-5, 5, -1.5]} angle={0.03} penumbra={1} intensity={4} castShadow shadow-mapSize={[1024, 1024]} color="#f5e2b2" />
      <spotLight position={[5, 5, -5]} angle={0.3} penumbra={1} intensity={1} castShadow={true} shadow-mapSize={[256, 256]} color="#fef0cd" />
      <Suspense fallback={null}>
        <NikeAirJordan />
        <ContactShadows frames={1} rotation-x={[Math.PI / 2]} position={[0, -0.4, 0]} far={0.4} width={2} height={2} blur={4} />
      </Suspense>
    </Canvas>
  )
}
